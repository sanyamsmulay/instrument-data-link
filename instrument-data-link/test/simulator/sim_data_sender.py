...
class SimConnectRecv:
    ...
    def pack(self) -> bytes:
        return struct.pack('iii', self.dwSize, self.dwVersion, self.dwID)
...
class SimConnectRecvEvent(SimConnectRecv):
    ...
    def pack(self) -> bytes:
        base = super().pack()
        return base + struct.pack('iii', self.uGroupID, self.uEventID, self.dwData)
...
class SimDataSender:
    ...
    def send_event(self, event_id: int, data: int = 0) -> None:
        """Send a simple event packet"""
        packet = SimConnectRecvEvent(
            dwSize=20,      # Size of SimConnectRecv (12) + Event data (8)
            dwVersion=0,    # Version is usually 0
            dwID=SIMCONNECT_RECV_ID_EVENT,
    ...
    def send_simobject_data(self, data: List[float]) -> None:
        ...
        print(f"Sending simulated aircraft data: {data}")

        base_size = 40  # Size of SimConnectRecv (12) + SimObjectData header (28)
    ...
    def run_test_sequence(self, interval: float = 10.0) -> None:
        ...
        try:
            ...
            self.send_event(EVENT_SIM_START)
